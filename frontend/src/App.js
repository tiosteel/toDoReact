...
const App = () => {
    ...
    const [currentTasks, setCurrentTasks] = React.useState(JSON.parse(localStorage.getItem("currentTasks")) || []);
        ...
        <Navbar variant="light" bg="light" expanded="true">
            <Container>
                <Col>
                    <Navbar.Brand>
                        <a href="/">
                            <img src="Wiley-logo.png" width="20" height="20"
                    ...
                    <Navbar.Collapse>
                        <Form className="ml-auto" inline>
                            ...
                            <Button variant="primary"
                                    ...
                                    onClick={(event) => handleAddTask(event)}>Add</Button>
                        </Form>
                    </Navbar.Collapse>
                </Col>
            </Container>
                ...
                {currentTasks.map((oTask, iIndex) =>
                    <ListGroupItem key={iIndex}>
                        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                                                <span
                                                    style={{textDecoration: (oTask.completed) ? 'line-through' : 'None'}}>
                                                    {oTask.name}
                                                </span>
                            <span>
                                                    <Button
                                                        style={{display: (oTask.completed) ? 'None' : 'Inline-block'}}
                                                        variant="success"
                                                        onClick={(event) => handleCompleteTask(iIndex, event)}>Complete</Button>
                                                    <Button
                                                        style={{display: (oTask.completed) ? 'Inline-block' : 'None'}}
                                                        variant="warning"
                                                        onClick={(event) => handleRestoreTask(iIndex, event)}>Restore</Button>{' '}
                                <Button variant="danger"
                                        onClick={(event) => handleDeleteTask(iIndex, event)}>Delete</Button>
                                                </span>
                        </div>
                    </ListGroupItem>
                )}
